import { Tab } from "@/entities/tabs/model/types";
import { useContextMenu } from "@/shared/ui/ContextMenu";

export const useContextMenuTabs = ({tabs, closeTab}: {tabs: Tab[], closeTab: (path: string)=> void}) => {
  ...
  const contextItems =
    ...
    return {
        contextMenu, contextItems, menuRef, closeMenu, handleContextMenu
    }
}